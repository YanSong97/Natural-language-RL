...
def collect_batch_rollout_single_player(
    ...
    for batch_idx in tqdm(range(num_rollout // batch_sample_size), desc="Collecting"):
        ...
        while not all(dones):
            ...
            action, responses = batch_policy(current_state, available_actions_list=[[0,1,2,3] for state in current_state], return_cot=True)      # default available action for FrozenLake
            # import pdb
            # pdb.set_trace()
            ...
            for i, (env, done) in enumerate(zip(env_list, dones)):
                ...
                traj_data[i]["reward"].append(reward)
                traj_data[i]['cot'].append(responses[action_id])
                states[i] = next_state
                action_id += 1
                if done:
                    dones[i] = True
